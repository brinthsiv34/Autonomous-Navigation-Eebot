; Abesa Sivananthanathan, Abishaa Thiyagendran, Brinthavi Sivarajasingham
;****************************************************************************
...
LNE_VRNCE 	FCB $12 ; E-F         ;84 	;DARK-[(DARK+LIGHT)/2]
BW_VRNCE  	FCB $4D ; A           ;2C
PRT_VRNCE 	FCB $1A ; B           ;76
MID_VRNCE 	FCB $46 ; C           ;46
STRBRD_VRNCE  FCB $33  ;D         ;66
...
; might need to move depending on how much space the stuff above takes from $3800

          	ORG $3850 ; Where our TOF counter register lives
TOF_COUNTER   dc.b 0 ; The timer, incremented at 23Hz
CRNT_STATE  	dc.b 2 ; Current state register
T_TURN       	ds.b 1 ; TURN time
...
; subrotine section
;---------------------------------------------------------------------------
DISPATCHER  JSR   CHECK_START                    	; Start of the Dispatcher
        	RTS



CHECK_START CMPA #START ; If it’s the START state -----------------
          	BNE START_FWD ; |
...
REV_TRN_CHECK CMPA #REV_TRN
            	BNE CHECK_LFT
            	JSR REV_TRN_ST
            	RTS
         	 
CHECK_STOP  CMPA #ALL_STP
        	BNE CHECK_LFT_TRN
        	JSR ALL_STP_ST
        	RTS
       	 
CHECK_LFT_TRN CMPA #LFT_TRN
          	BNE CHECK_RGHT_TRN
          	JSR LFT
          	RTS
         	 

CHECK_LFT   CMPA #LFT_ALGN
        	BNE CHECK_RGHT
        	JSR LFT_DONE
        	RTS
       	 
CHECK_RGHT_TRN  CMPA #RGHT_TRN
             	BNE REV_TRN_CHECK
             	RTS
            	 
CHECK_RGHT  CMPA #RGHT_ALGN
        	JSR RGHT_DONE
        	RTS

;DISP_EXIT   RTS ; Exit from the state dispatcher ----------
;*******************************************************************
START_ST  	BRCLR PORTAD0,$04, START_EXIT   ;$04
          	JSR INIT_FWD
          	MOVB #FWD, CRNT_STATE
...
NO_FWD_BUMP BRSET PORTAD0,$04,NO_REARFWD_BUMP ; If REAR_BUMP, then we should stop
          	MOVB #ALL_STP,CRNT_STATE ; and change state to ALL_STOP
          	JSR INIT_ALL_STP ; so initialize the ALL_STOP state
          	LBRA EXIT ; and return
         	 
NO_REARFWD_BUMP LDAA SENSOR_BOW
            	ADDA BW_VRNCE
            	CMPA BSE_BW
            	BPL  NT_ALGN
            	LDAA SENSOR_MID
...
NT_ALGN 	LDAA SENSOR_PORT
        	ADDA PRT_VRNCE
        	CMPA BSE_PRT
        	BPL PART_LFT_TRN
        	BMI NO_PRT
       	 
NO_PRT  	LDAA SENSOR_BOW
        	ADDA BW_VRNCE
        	CMPA BSE_BW
        	BPL EXIT
        	BMI NO_BW

NO_BW   	LDAA SENSOR_STBD
        	ADDA STRBRD_VRNCE
        	CMPA BSE_STB
        	BPL PART_RGHT_TRN
        	BMI EXIT
       	 
PART_LFT_TRN LDY #9750                                                          	 
           	JSR del_50us 	 
           	JSR INIT_LFT
...
CHCK_LFT	JSR INIT_LFT
        	MOVB #LFT_ALGN, CRNT_STATE
        	BRA EXIT
       	 
       	 
PART_RGHT_TRN LDY #9750                                                         	 
          	JSR del_50us 	 
          	JSR INIT_RGHT
          	MOVB #RGHT_TRN, CRNT_STATE
          	LDY #9750                                                            	 
          	JSR del_50us 	 
          	BRA EXIT
         	 
         	 
CHCK_RGHT JSR INIT_RGHT
      	  MOVB #RGHT_ALGN, CRNT_STATE
        	BRA EXIT 	 
...
LFT_DONE  MOVB #FWD, CRNT_STATE
      	JSR INIT_FWD
      	BRA EXIT
     	 
RGHT  	LDAA SENSOR_BOW
      	ADDA BW_VRNCE
      	CMPA BSE_BW
      	BPL RGHT_DONE
      	BMI EXIT
     	 
RGHT_DONE MOVB #FWD, CRNT_STATE
      	JSR INIT_FWD
      	BRA EXIT
     	 

;*******************************************************************
REV_TRN_ST  LDAA SENSOR_BOW ; If Tc>Trevturn then
      	ADDA BW_VRNCE
         	CMPA BSE_BW ; the robot should go FWD
      	BMI EXIT
        	JSR INIT_LFT ; initialize the FWD state
        	MOVB #FWD,CRNT_STATE ; set state to FWD
        	JSR INIT_FWD
        	BRA EXIT

ALL_STP_ST  BRSET PORTAD0,%00000100,ALL_STP_EXIT ; If FWD_BUMP  $04
        	MOVB #START,CRNT_STATE ; set the state to START
ALL_STP_EXIT RTS ; return to the MAIN routine

;*******************************************************************
INIT_RGHT BSET PORTA, %00000010
      	BCLR PORTA, %00000001
      	LDAA TOF_COUNTER
      	ADDA #RIGHT_TIMER
      	STAA T_TURN
